...
int BOOTSD_Ioctl(int function, void *param)
{
	...
	switch(function)
	{
		...
		case DEV_BOOTCODE_CA7_READ_PAGE:
			{
				int retVal = 0;
				ioctl_diskrwpage_t	*rw	 =	(ioctl_diskrwpage_t *)param;
				//unsigned int uiBootStart = (unsigned int)BOOTSD_GetBootStart(rw->boot_partition);

				retVal = BOOTSD_Read(iSlotIndex, rw->start_page, rw->rw_size, rw->buff);
		...
		case DEV_BOOTCODE_CA7_WRITE_PAGE:
			{
				int retVal=0;
				ioctl_diskrwpage_t	*rw	 =	(ioctl_diskrwpage_t *)param;
				//unsigned int uiBootStart = (unsigned int)BOOTSD_GetBootStart( rw->boot_partition );

				if(BOOTSD_Write(iSlotIndex, rw->start_page, rw->rw_size, rw->buff) < 0)
				{
					retVal = BOOTSD_Write(iSlotIndex, rw->start_page, rw->rw_size, rw->buff);
				}
		...
		case DEV_GET_DISK_SIZE:
			{
				...
				if (SD_BUS_IsMemoryCard(iSlotIndex))
				{
					...
					*pulSectorSize += 1;
#endif
					printf("[%s] pulSectorSize : 0x%08x\n", __func__, *pulSectorSize);
