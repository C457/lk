...
static long get_bitmap_offset(enum image_type type)
{
	switch (type) {
		case BOOT_LOGO:
			return OFFSET_BOOT_LOGO;

		case PARKING_GUIDE_LOGO:
			return OFFSET_PARKING_GUIDE_LOGO;

		case MAKE_QUICKBOOT_LOGO:
			return OFFSET_MAKE_QUICKBOOT_LOGO;

		case BLUELINK_LOGO:
			return OFFSET_BLUELINK_LOGO;
...
static int bitmap_buffer_to_dest(long ptn_offset, struct dest_image_buffer *dbuf)
{
	...
	ptn_index = partition_get_index("splash");
	...
	//read emmc
	tcc_read(ptn + ptn_offset, buf, page_size);
	...
	tcc_read(ptn + ptn_offset, buf, read_size);
 ...
 * @return	0 is success, -1 is failed.
 */
int bitmap_to_addr(enum image_type type, unsigned int pg_addr, unsigned int *width, unsigned int *height)
{
	...
	*width = dbuf.width = WIDTH_DEFAULT_DEST;
	*height = dbuf.height = HEIGHT_DEFAULT_DEST;
