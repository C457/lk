		...
		unsigned int boot_partition)
{
	...
	if (!(buf[0] == 'B' && buf[1] == 'L' && (buf[2] == 'x' || buf[2] == 'p' || buf[2] == 'w')))
#endif
	{
...
#if defined(TSBM_ENABLE)
  BOOTSD_sHeader_Info_T *secure_h = (BOOTSD_sHeader_Info_T*) ROMFILE_TEMP_BUFFER;
  if (!memcmp((char*)secure_h->sc_tag, SECURE_TAG, SECURE_TAG_SIZE))
  {
    return fb_mmc_update_secure_bootloader((void*)ROMFILE_TEMP_BUFFER, rom_size, boot_partition);
  }
#endif
...
#if defined(TSBM_ENABLE)
int fb_mmc_update_secure_bootloader(void *buffer, unsigned int rom_size, int boot_partition)
{
...
void update_rbid(void)
{
	...
	for (boot_partition = 0; boot_partition < 2; boot_partition++)
	{
		printf("[Update RBID] Partition=%d, (%x)\n", boot_partition, BOOTSD_GetHeaderAddr( boot_partition ));

		mmc_init_page( &p, BOOTSD_GetHeaderAddr( boot_partition ), 1, 0, pTempData, boot_partition);//ks_8930
		DISK_Ioctl(DISK_DEVICE_TRIFLASH, DEV_BOOTCODE_READ_PAGE, (void*)&p);

		memcpy(&header, pTempData, 512);

		mmc_init_page( &p, header.ulConfigCodeBase, 1, 0, pTempData, boot_partition);//ks_8930
		DISK_Ioctl(DISK_DEVICE_TRIFLASH, DEV_BOOTCODE_READ_PAGE, (void*)&p);
		rbid = tcsb_api_imgParseAttribute(pTempData, 0x2);
		tcsb_otp_set_count(0, boot_partition, rbid);
		printf("Config RBID %d\n", rbid);

		mmc_init_page( &p, header.ulBootloaderBase, 1, 0, pTempData, boot_partition);//ks_8930
		DISK_Ioctl(DISK_DEVICE_TRIFLASH, DEV_BOOTCODE_READ_PAGE, (void*)&p);
		rbid = tcsb_api_imgParseAttribute(pTempData, 0x2);
		tcsb_otp_set_count(1, boot_partition, rbid);
		printf("Bootloader RBID %d\n", rbid);
	}
}
