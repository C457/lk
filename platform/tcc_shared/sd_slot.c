...
static int SD_SLOT_WaitIntStatus(PSD_SLOT_T pSlot, unsigned short usIntStatus)
{
	...
	for(;;)
	{
		...
		if(usNormalIntStatus&HwSD_STS_ERR)
		{
			...
			printf("Command : 0x%04x, Transfer Mode : 0x%04x\n",
					pSdSlotReg->usCommand, pSdSlotReg->usTransferMode);
			printf("Response10 : 0x%08x, Response32 : 0x%08x\n",
					pSdSlotReg->ulResponse[0], pSdSlotReg->ulResponse[1]);
			printf("Response54 : 0x%08x, Response76 : 0x%08x\n",
					pSdSlotReg->ulResponse[2], pSdSlotReg->ulResponse[3]);
			printf("Power : 0x%02x, Host Control : 0x%02x\n",
					pSdSlotReg->ucPowerControl, pSdSlotReg->ucHostControl);
			printf("Normal Interrupt Status : 0x%04x%04x\n", 
					pSdSlotReg->usErrorIntStatus, pSdSlotReg->usNormalIntStatus);
			printf("Host Control2/ACMD12 Error Status : 0x%08x\n",
