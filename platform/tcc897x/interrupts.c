...
#define GIC_CPU_REG(off)	(HwARM_GIC_CPU_BASE  + (off))
#define GIC_DIST_REG(off)	(HwARM_GIC_DIST_BASE + (off))
...
static void platform_gic_dist_init(void)
{
	...
	for (i=0 ; i<32 ; i+=4) {
		reg_value = readl(GIC_DIST_TARGET + (i*4)/4);
	...
	reg_value |= (reg_value << 16);
	for (i=32 ; i<irqs ; i+=4)
		writel(reg_value, GIC_DIST_TARGET + (i*4)/4);
	for (i=32 ; i<irqs ; i+=4)
