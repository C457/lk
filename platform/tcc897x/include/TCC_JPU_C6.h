...
#define PHYSICAL_ADDR	0	// physical address
#define VIRTUAL_ADDR	1	// virtual  address
...
//  - main() 함수만 환경에 맞게 구성하고, jpu_c6_decoder() 는 호출만 하면 됩니다.
//	jpu_dec_inits_t 에 JPU 에서 필요한 필드 setting 해야 합니다.
//	jpu_dec_outputs_t 는 이용하셔야 할 output information 입니다.

//--=========================================================================--

typedef struct jpu_dec_inits_t 
{
	unsigned int register_base_virtual_address;	//!< register base virtual address (TCC897X : 0x75300000)
	unsigned int memory_base_physical_address;	//!< physical address (for bitstream buffer, frame buffer)
	unsigned int memory_base_virtual_address;	//!< virtual address (for bitstream buffer, frame buffer)
	int input_stream_size;						//!< input stream size for decoding

	//! Callback Func
	void* (*m_Memcpy ) ( void*, const void*, unsigned int );	//!< memcpy
	void  (*m_Memset ) ( void*, int, unsigned int );		//!< memset
	int   (*m_Interrupt ) ( void );					//!< interrupt
} jpu_dec_inits_t;

typedef struct jpu_dec_outputs_t 
{
	int m_iHeight;
	int m_iWidth;
	int m_iYUVFormat;			//  0(YUV420), 1(YUV422), 2(YUV224), 3(YUV444), 4(YUV400)
	unsigned char* m_pCurrOut[2][3];	//  output YUV buffer
	//	m_pCurrOut[0][0] : physical address of Luma
	//	m_pCurrOut[0][1] : physical address of Cb
	//	m_pCurrOut[0][2] : physical address of Cr
	//	m_pCurrOut[1][0] : virtual address of Luma
	//	m_pCurrOut[1][1] : virtual address of Cb
	//	m_pCurrOut[1][2] : virtual address of Cr
} jpu_dec_outputs_t;

extern jpu_dec_outputs_t jpu_out;
