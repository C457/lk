...
#include <i2c.h>
#include <debug.h>
#include <dev/pfuze100.h>

#define DUMP_PFUZE100 1

#ifndef ARRAY_SIZE
#define ARRAY_SIZE(x) (sizeof(x) / sizeof((x)[0]))
#endif

struct pfuze100_voltage_t {
	unsigned int uV;
	unsigned char val;
};

/* device address */
#define SLAVE_ADDR_PFUZE100	0x10 // SADDR connect to AGND
/* command registers */
#define PFUZE100_DEVICE_ID_REG				0x00
#define PFUZE100_BUCK1_CONTROL_REG			0x01
#define PFUZE100_BUCK2_CONTROL_REG			0x02
#define PFUZE100_BUCK3_CONTROL_REG			0x03
#define PFUZE100_BUCK4_CONTROL_REG			0x04
#define PFUZE100_VRC_SETTING_REG				0x05
#define PFUZE100_BUCK_MODE_REG				0x06
#define PFUZE100_LDO1_CONTROL_REG				0x07
#define PFUZE100_LDO2_CONTROL_REG				0x08
#define PFUZE100_LDO3_CONTROL_REG				0x09
#define PFUZE100_LDO4_CONTROL_REG				0x0A
#define PFUZE100_LDO5_CONTROL_REG				0x0B
#define PFUZE100_LDO6_CONTROL_REG				0x0C
#define PFUZE100_LDO7_CONTROL_REG				0x0D
#define PFUZE100_LDO8_CONTROL_REG				0x0E
#define PFUZE100_BUCK_ON_OFF_REG				0x12
#define PFUZE100_LDO_ON_OFF_REG				0x13



/* BUCK voltage level */
static struct pfuze100_voltage_t buck12_voltages[] = {
...
static struct pfuze100_voltage_t buck34_voltages[] = {
...
static struct pfuze100_voltage_t ldo12378_voltages[] = {
...
static struct pfuze100_voltage_t ldo456_voltages[] = {
...
static int pfuze100_initialized = 0;
static int pfuze100_i2c_ch = 2;
static unsigned char pfuze100_read(unsigned char cmd)
{
	unsigned char recv_data;
	if(	i2c_xfer(SLAVE_ADDR_PFUZE100, 1, &cmd, 1, &recv_data, pfuze100_i2c_ch) != 0) {
		dprintf(INFO,"[PMIC] failed i2c_xfer\r\n");
	}
	return recv_data;
}

static unsigned char pfuze100_read2(unsigned char id, unsigned char cmd)
{
	unsigned char recv_data;
	if(	i2c_xfer(id, 1, &cmd, 1, &recv_data, pfuze100_i2c_ch) != 0) {
		dprintf(INFO,"failed i2c_xfer\r\n");
	}
	return recv_data;
}

static void pfuze100_write(unsigned char cmd, unsigned char value)
{
	unsigned char send_data[2];
	send_data[0] = cmd;
	send_data[1] = value;
	i2c_xfer(SLAVE_ADDR_PFUZE100, 2, send_data, 0, 0, pfuze100_i2c_ch);
}

#if defined(DUMP_PFUZE100)
void pfuze100_dump_buck(void)
{
	unsigned char reg, data;

	unsigned int i, uV, max_num = 0;

	static struct pfuze100_voltage_t *buck_voltags;

	
	for(reg = PFUZE100_BUCK1_CONTROL_REG; reg <= PFUZE100_BUCK4_CONTROL_REG;reg++) {
		data = pfuze100_read(reg);

		switch (reg) {
			case PFUZE100_BUCK1_CONTROL_REG:
				buck_voltags = buck12_voltages;
				max_num = NUM_BUCK12;
				break;
			case PFUZE100_BUCK2_CONTROL_REG:
				buck_voltags = buck12_voltages;
				max_num = NUM_BUCK12;
				break;
			case PFUZE100_BUCK3_CONTROL_REG:
				buck_voltags = buck34_voltages;
				max_num = NUM_BUCK34;
				break;
			case PFUZE100_BUCK4_CONTROL_REG:
				buck_voltags = buck34_voltages;
				max_num = NUM_BUCK34;
				break;
			default:
		...
		dprintf(INFO,"BUCK[%d] = 0x%04x (%d)uV\r\n", reg - PFUZE100_BUCK1_CONTROL_REG + 1,  data >> 2, uV);
	}
		
}

void pfuze100_dump_ldo(void)
{
	unsigned char reg, data;

	unsigned int i, uV, max_num = 0;
	
	static struct pfuze100_voltage_t *ldo_voltags;
	
	for(reg = PFUZE100_LDO2_CONTROL_REG; reg <= PFUZE100_LDO8_CONTROL_REG;reg++) {
		data = pfuze100_read(reg);

		switch (reg) {
			case PFUZE100_LDO2_CONTROL_REG:
				ldo_voltags = ldo12378_voltages;
				max_num = NUM_LDO12378;
				break;
			case PFUZE100_LDO3_CONTROL_REG:
				ldo_voltags = ldo12378_voltages;
				max_num = NUM_LDO12378;
				break;
			case PFUZE100_LDO4_CONTROL_REG:
				ldo_voltags = ldo456_voltages;
				max_num = NUM_LDO456;
				break;
			case PFUZE100_LDO5_CONTROL_REG:
				ldo_voltags = ldo456_voltages;
				max_num = NUM_LDO456;
				break;
			case PFUZE100_LDO6_CONTROL_REG:
				ldo_voltags = ldo456_voltages;
				max_num = NUM_LDO456;
				break;
			case PFUZE100_LDO7_CONTROL_REG:
				ldo_voltags = ldo12378_voltages;
				max_num = NUM_LDO12378;
				break;
			case PFUZE100_LDO8_CONTROL_REG:
				ldo_voltags = ldo12378_voltages;
				max_num = NUM_LDO12378;
				break;
			default:
		...
		dprintf(INFO,"[PMIC] LDO[%d] = 0x%04x (%d)uV\r\n", reg - PFUZE100_LDO1_CONTROL_REG + 1,  data, uV);
	}
...
void pfuze100_init(int i2c_ch)
{
	int i;
	char data;
	pfuze100_i2c_ch = i2c_ch;
	pfuze100_initialized = 1;

#if defined(DUMP_PFUZE100)
	data = pfuze100_read(PFUZE100_DEVICE_ID_REG);
	dprintf(INFO,"[PMIC] pfuze100_init PFUZE100 DEVID=0x%x\r\n", data);
	
//	pfuze100_write(0x20, 0x1F); //requested by lee jung-yeol
	data = pfuze100_read(0x20);
	dprintf(INFO,"[PMIC] ADDR 0x20 = 0x%x\r\n", data);

    //buffalo+ 
	pfuze100_write(0x66, 0x4A); //requested by kwak ki-tae 
	data = pfuze100_read(0x66);
    dprintf(INFO,"[PMIC] ADDR 0x66 = 0x%x\r\n", data);
	//buffalo-
...
// ON OFF
static int pfuze100_output_control(pfuze100_src_id id, unsigned int onoff)
{
	unsigned char reg, old_value, value, bit;

	switch (id){	
		case PFUZE100_ID_BUCK1:
		case PFUZE100_ID_BUCK2:
		case PFUZE100_ID_BUCK3:
		case PFUZE100_ID_BUCK4:
			reg = PFUZE100_BUCK_ON_OFF_REG;
			bit = id-PFUZE100_ID_BUCK1;
			break;
		case PFUZE100_ID_LDO2:
		case PFUZE100_ID_LDO3:
		case PFUZE100_ID_LDO4:
		case PFUZE100_ID_LDO5:
		case PFUZE100_ID_LDO6:
		case PFUZE100_ID_LDO7:
		case PFUZE100_ID_LDO8:
			reg = PFUZE100_LDO_ON_OFF_REG;
			bit = id-PFUZE100_ID_LDO1;
			break;
		default:
			return -1;
	}

	old_value = pfuze100_read(reg);
	if (onoff)
		value = (old_value | (1<<bit));
	else
		value = (old_value & ~(1<<bit));

	dprintf(INFO, "[PMIC] %s: id:%d, onoff:%d, reg:0x%x, value:0x%x\n", __func__, id, onoff, reg, value);
	pfuze100_write(reg, value);

	return 0;
}

static int pfuze100_buck_set_voltage(pfuze100_src_id id, unsigned int uV)
{
	...
	switch (id) {
		case PFUZE100_ID_BUCK1:
			reg = PFUZE100_BUCK1_CONTROL_REG;
			buck_voltags = buck12_voltages;
			max_num = NUM_BUCK12;
			break;
		case PFUZE100_ID_BUCK2:
			reg = PFUZE100_BUCK2_CONTROL_REG;
			buck_voltags = buck12_voltages;
			max_num = NUM_BUCK12;
			break;
		case PFUZE100_ID_BUCK3:
			reg = PFUZE100_BUCK3_CONTROL_REG;
			buck_voltags = buck34_voltages;
			max_num = NUM_BUCK34;
			break;
		case PFUZE100_ID_BUCK4:
			reg = PFUZE100_BUCK4_CONTROL_REG;
			buck_voltags = buck34_voltages;
			max_num = NUM_BUCK34;
			break;
		default:
	...
	dprintf(INFO, "[PMIC] %s: id:%d, uV:%d, reg:0x%x, value:0x%x\n", __func__, id, uV, reg, value);
	pfuze100_write(reg, value);
	pfuze100_output_control(id, 1);
	return 0;
}

static int pfuze100_ldo_set_voltage(pfuze100_src_id id, unsigned int uV)
{
	...
	switch (id) {
		case PFUZE100_ID_LDO2:
			reg = PFUZE100_LDO2_CONTROL_REG;
			ldo_voltags = ldo12378_voltages;
			max_num = NUM_LDO12378;
			break;
		case PFUZE100_ID_LDO3:
			reg = PFUZE100_LDO3_CONTROL_REG;
			ldo_voltags = ldo12378_voltages;
			max_num = NUM_LDO12378;
			break;
		case PFUZE100_ID_LDO4:
			reg = PFUZE100_LDO4_CONTROL_REG;
			ldo_voltags = ldo456_voltages;
			max_num = NUM_LDO456;
			break;
		case PFUZE100_ID_LDO5:
			reg = PFUZE100_LDO5_CONTROL_REG;
			ldo_voltags = ldo456_voltages;
			max_num = NUM_LDO456;
			break;
		case PFUZE100_ID_LDO6:
			reg = PFUZE100_LDO6_CONTROL_REG;
			ldo_voltags = ldo456_voltages;
			max_num = NUM_LDO456;
			break;
		case PFUZE100_ID_LDO7:
			reg = PFUZE100_LDO7_CONTROL_REG;
			ldo_voltags = ldo12378_voltages;
			max_num = NUM_LDO12378;
			break;
		case PFUZE100_ID_LDO8:
			reg = PFUZE100_LDO8_CONTROL_REG;
			ldo_voltags = ldo12378_voltages;
			max_num = NUM_LDO12378;
			break;
		default:
...
int pfuze100_set_voltage(pfuze100_src_id id, unsigned int mV)
{
	dprintf(INFO,"[PMIC] pfuze100_set_voltage : %d\n", mV);
	if (pfuze100_initialized == 0)
		return -1;

	/* power off */
	if (mV == 0)
		pfuze100_output_control(id, 0);
	else {
		switch (id){
			case PFUZE100_ID_BUCK1:
			case PFUZE100_ID_BUCK2:
			case PFUZE100_ID_BUCK3:
			case PFUZE100_ID_BUCK4:
				pfuze100_buck_set_voltage(id, mV*1000);
				break;
			case PFUZE100_ID_LDO2:
			case PFUZE100_ID_LDO3:
			case PFUZE100_ID_LDO4:
			...
			case PFUZE100_ID_LDO7:
			case PFUZE100_ID_LDO8:
				pfuze100_ldo_set_voltage(id, mV*1000);
				break;
			default:
				return -1;
		}
...
int pfuze100_set_power(pfuze100_src_id id, unsigned int onoff)
{
	if (pfuze100_initialized == 0)
		return -1;
